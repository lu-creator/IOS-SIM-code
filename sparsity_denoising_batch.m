...
file_name=FileName_batch{i_file};
disp(['Sparsity denoising the data :  ',file_name ,' ,please wait...']);
y=imreadstack(strcat(pathname,file_name));%读入第i_file个维纳结果
% axes1显示，清空axes2
axes(handles.axes1);
imshow(y(:,:,1),[]);
cla(handles.axes2);
...
imwritestack_16(tif, [pathname '\denoise-' file_name]);
close(Progressbar);

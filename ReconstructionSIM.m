...
addpath('./TIF')
%设置频谱p的初始范围 
% mask 3.62   488:178,561:155,637:136 
% seek_range = 5;
% if wavelength == 488     
%     init_position = 196; 
% elseif wavelength == 561
%     init_position = 171;  
% elseif wavelength == 637 || wavelength == 638 || wavelength == 640
%     init_position = 150;
% else
%     init_position = 160;
%     seek_range = 60;
...
elseif using_parameter==0
    ...
    %         determine_p(image_stack, psf, sp_rate, num_of_average, max_h_w, fre_length_half, seek_range, init_position ,...
    ...
    [phase_matrix, zuobiaox, zuobiaoy, peakmax]=...
        ...
        nphases, nangles, Progressbar);
    % 20221102 Wenjy test：坐标按照波长缩放
%     load('D:\03_TestData\20221028_ParamP\test\结果reconstruction Wiener sim1\488_zuobiao.mat')
%     zuobiaox = zuobiaoxAvg;
%     zuobiaoy = zuobiaoyAvg;
%     for i = 1:9
%         if i ~= 1 && i ~= 4 && i ~= 7
%             if zuobiaoxAvg(i) > zuobiaoxAvg(1)
%                 zuobiaox(i) = zuobiaoxAvg(1) + sqrt((zuobiaoyAvg(i)-zuobiaoyAvg(1)).^2+(zuobiaoxAvg(i)-zuobiaoxAvg(1)).^2).*(488/wavelength).*cos(abs(atan((zuobiaoyAvg(i)-zuobiaoyAvg(1))./(zuobiaoxAvg(i)-zuobiaoxAvg(1)))));
%             else
%                 zuobiaox(i) = zuobiaoxAvg(1) - sqrt((zuobiaoyAvg(i)-zuobiaoyAvg(1)).^2+(zuobiaoxAvg(i)-zuobiaoxAvg(1)).^2).*(488/wavelength).*cos(abs(atan((zuobiaoyAvg(i)-zuobiaoyAvg(1))./(zuobiaoxAvg(i)-zuobiaoxAvg(1)))));
%             end
%             if zuobiaoyAvg(i) > zuobiaoyAvg(1)
%                 zuobiaoy(i) = zuobiaoyAvg(1) + sqrt((zuobiaoyAvg(i)-zuobiaoyAvg(1)).^2+(zuobiaoxAvg(i)-zuobiaoxAvg(1)).^2).*(488/wavelength).*sin(abs(atan((zuobiaoyAvg(i)-zuobiaoyAvg(1))./(zuobiaoxAvg(i)-zuobiaoxAvg(1)))));
%             else
%                 zuobiaoy(i) = zuobiaoyAvg(1) - sqrt((zuobiaoyAvg(i)-zuobiaoyAvg(1)).^2+(zuobiaoxAvg(i)-zuobiaoxAvg(1)).^2).*(488/wavelength).*sin(abs(atan((zuobiaoyAvg(i)-zuobiaoyAvg(1))./(zuobiaoxAvg(i)-zuobiaoxAvg(1)))));
%             end
%         end
%     end
%     zuobiaox = zuobiaox.*(max_h_w/zuobiaox(1));
%     zuobiaoy = zuobiaoy.*(max_h_w/zuobiaoy(1));
%     [c6, angle6, g_hist_output, g_hist_fitting_output]...
%         = determine_phase_Origin(image_stack, zuobiaox, zuobiaoy, bgjie488, phase_matrix,...
%         num_of_average, fc_ang, fc_con, wavelength, psf, nphases, nangles, Progressbar, isbatch);
%     load('D:\03_TestData\20221022_GaussIllumination\NA0.5\poisson\结果reconstruction Wiener sim\GT_3000_zuobiao.mat')
%     zuobiaox = zuobiaox.*(max_h_w/zuobiaox(1));
%     zuobiaoy = zuobiaoy.*(max_h_w/zuobiaoy(1));
%     [isPTrue, ~, ~, ~] = evaluateP(zuobiaox, zuobiaoy);
%     if ~isPTrue
%         if wavelength == 488
%             load('D:\03_TestData\GTL_SIM\20230615_SIM_Mask400\ROI2\ROI_512\结果reconstruction Wiener sim\计算\488_zuobiao.mat')
%         elseif wavelength == 561
%             load('D:\03_TestData\GTL_SIM\20230615_SIM_Mask400\ROI2\ROI_512\结果reconstruction Wiener sim\计算\561_zuobiao.mat')
%         elseif wavelength == 637 || wavelength == 638 || wavelength == 640
%             load('D:\03_TestData\GTL_SIM\20230615_SIM_Mask400\ROI2\ROI_512\结果reconstruction Wiener sim\计算\637_zuobiao.mat')
%         end
%     end
%     load('D:\02_数据中转站\ParamP\20231011\结果reconstruction Wiener sim\488_2DSIM-3_zuobiao.mat')
    % load('D:\03_TestData\重建问题数据\20240110_点\488_2DSIM-2_zuobiao.mat');
    [c6, angle6, g_hist_output, g_hist_fitting_output]...
    ...
    if phase_plot_enable==1
        for ii_polt=1:6
            ...
            print(gcf, '-dtiff', '-r300', [pathname_superior '\结果reconstruction Wiener sim\' filename(1:end-4), '_fitting', num2str(ii_polt), '.png']);
        end
    end
end

% % 保存新参数
save([pathname_superior '\结果reconstruction Wiener sim\' filename(1:end-4) '_zuobiao.mat'],'zuobiaox','zuobiaoy');
save([pathname_superior '\结果reconstruction Wiener sim\' filename(1:end-4) '_samec_angle.mat'],'c6','angle6');
...
%         if iframe == 1
%             imwritestack_16(stack, [pathname_superior '\结果reconstruction Wiener sim\Wiener' filename]);
%             imwritestack( abs(drr),[pathname_superior  '\结果reconstruction Wiener sim\' filename(1:end-4),'-fft.tif']);
%         else
%             imwritestacka_16(stack, [pathname_superior '\结果reconstruction Wiener sim\Wiener' filename]);
%             imwritestacka( abs(drr),[pathname_superior  '\结果reconstruction Wiener sim\' filename(1:end-4),'-fft.tif']);
%         end
    end
